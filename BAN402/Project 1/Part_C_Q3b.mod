...
param port_capacity {Ports};           # Capacity limits at ports (tons)
...
# Amount transported from regions to ports cannot exceed capacity limits
subject to port_limit {p in Ports}: 
	sum {r in Regions} x[r,p] <= port_capacity[p];
